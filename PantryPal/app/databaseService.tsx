...
const predefinedCategories = [
    ...
    'Health & Supplements', 'Miscellaneous'
];
...
export const initializeDatabase = () => {
    try {
        // Create tables
        db.execSync(`CREATE TABLE IF NOT EXISTS Categories (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT);`);
        db.execSync(`CREATE TABLE IF NOT EXISTS Brands (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT);`);
        db.execSync(`CREATE TABLE IF NOT EXISTS Allergens (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT);`);
        db.execSync(`CREATE TABLE IF NOT EXISTS PantryItemAllergens (id INTEGER PRIMARY KEY AUTOINCREMENT, item_id INTEGER, allergen_id INTEGER, FOREIGN KEY(item_id) REFERENCES PantryItems(id), FOREIGN KEY(allergen_id) REFERENCES Allergens(id));`);
        db.execSync(`CREATE TABLE IF NOT EXISTS Locations (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT);`);
        db.execSync(`CREATE TABLE IF NOT EXISTS PantryItems (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, category_id INTEGER, brand_id INTEGER, location_id INTEGER, expiration_date DATE, quantity INTEGER, added_date DATE, notes TEXT, FOREIGN KEY(category_id) REFERENCES Categories(id), FOREIGN KEY(brand_id) REFERENCES Brands(id), FOREIGN KEY(location_id) REFERENCES Locations(id));`);
        db.execSync(`CREATE TABLE IF NOT EXISTS ShoppingList (id INTEGER PRIMARY KEY AUTOINCREMENT, item_name TEXT, quantity INTEGER, added_date DATE, notes TEXT);`);

        // Insert predefined Categories if empty
        const categoryCount = db.getAllSync<CountRow>('SELECT COUNT(*) as count FROM Categories;', [])[0].count;
        if (categoryCount === 0) {
        ...
        const allergenCount = db.getAllSync<CountRow>('SELECT COUNT(*) as count FROM Allergens;', [])[0].count;
        if (allergenCount === 0) {
